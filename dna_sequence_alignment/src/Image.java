 ...
 * supports accessing the color of a specific pixel.
 */
...
public class Image {
  ...
  /**
   * Return a quantized version of this image where the given number
   * of lower-order bits of each color component are masked to zeroes.
   */
  ...
  public Image quantize(int bitsPerChannel) {
    ...
    for (int x = 0; x < getWidth(); x++)
      for (int y = 0; y < getHeight(); y++)
        copy.setColor(x, y, 
                      new TinyColor(getColor(x, y), 
                                    bitsPerChannel).expand());
    return copy;
  }
