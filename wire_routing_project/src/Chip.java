...
public class Chip {
  ...
  public Chip(File file) {
    try {
      ...
      /**
       * Read the wire data and create/initialize wires.
       */
      ...
      for (int i = 0; i < numWires; i++) {
        String line = bufferedReader.readLine();
        String tokens[] = line.split(" ");
        wires.add(new Wire(i + 1, new Coord(tokens[0], tokens[1]),
                new Coord(tokens[2], tokens[3])));
      }
  ...
  public boolean isFree(Coord coord) { return grid.get(coord) == Constants.FREE;
  }
